...
namespace VisualLogger.Contents
{
    public class LogContentText : LogContent<LogSchemaText, LogSchemaText.BlockText, LogSchemaText.BodyText, LogSchemaText.CellText>
    {
            ...
            ref long streamPosition)
        {
            ...
            while (line != null || isItemCreating)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < block.Cells.Length; i++)
                    {
                        ...
                        var streamCell = CreateStreamCell(logContent, mixStreamReader, match, cell, ref streamPosition);
            ...
            return new BlockContent(block.Name, cells);
        }
            ...
            ref long streamPosition)
        {
            ...
            var streamCell = new StreamCell(mixStreamReader, startPosition, capture.Length, StreamCellType.String, logContent.GetConvertor(cell.ConvertorName));
            return streamCell;
        }

        protected override BodyContent CreateBodyContent(ILogContent logContent, MixStreamReader mixStreamReader, LogSchemaText.BodyText body, ref long streamPosition)
        {
            ...
            while (line != null || isItemCreating)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < item.Length; i++)
                    {
                        ...
                        var streamCell = CreateStreamCell(logContent, mixStreamReader, match, template, ref streamPosition);
                        item[i] = streamCell;
                    }
                    items.Add(item);
                    streamPosition = startPosition;
            ...
            return new BodyContent(body.BodyTemplate.Select(t => t.Name).ToArray(), items);
        }
    }
}
