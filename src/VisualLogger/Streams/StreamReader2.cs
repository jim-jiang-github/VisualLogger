...
#nullable disable
    ...
    public class StreamReader2 : TextReader
    {
        // StreamReader2.Null is threadsafe.
        public new static readonly StreamReader2 Null = new NullStreamReader();
        ...
        // disposing of this StreamReader2.  A name like _leaveOpen is better, 
...
#if FEATURE_ASYNC_IO
        // We don't guarantee thread safety on StreamReader2, but we should at 
        ...
        // StreamReader2 by default will ignore illegal UTF8 characters. We don't want to 
        // throw here because we want to be able to read ill-formed data without choking. 
        // The high level goal is to be tolerant of encoding errors when we read and very strict 
        // when we write. Hence, default StreamWriter encoding will throw on error.   

        internal StreamReader2()
        {
        }

        public StreamReader2(Stream stream)
            : this(stream, true)
        {
        }

        public StreamReader2(Stream stream, bool detectEncodingFromByteOrderMarks)
            : this(stream, Encoding.UTF8, detectEncodingFromByteOrderMarks, DefaultBufferSize, false)
        {
        }

        public StreamReader2(Stream stream, Encoding encoding)
            : this(stream, encoding, true, DefaultBufferSize, false)
        {
        }

        public StreamReader2(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks)
            : this(stream, encoding, detectEncodingFromByteOrderMarks, DefaultBufferSize, false)
        {
        }

        // Creates a new StreamReader2 for the given stream.  The 
        ...
        // 
        public StreamReader2(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize)
            : this(stream, encoding, detectEncodingFromByteOrderMarks, bufferSize, false)
        {
        }

        public StreamReader2(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize, bool leaveOpen)
        {
        ...
        public StreamReader2(String path)
            : this(path, true)
        {
#if FEATURE_LEGACYNETCF
            if(CompatibilitySwitches.IsAppEarlierThanWindowsPhone8) {
                ...
                if(callingAssembly != null && !callingAssembly.IsProfileAssembly) {
                    ...
                    throw new MethodAccessException(String.Format(
                        ...
                        "Arg_MethodAccessException_WithCaller"),
                        caller,
                        callee));
                }
            }
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public StreamReader2(String path, bool detectEncodingFromByteOrderMarks)
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public StreamReader2(String path, Encoding encoding)
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public StreamReader2(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks)
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        public StreamReader2(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize)
        ...
        [ResourceExposure(ResourceScope.Machine)]
        [ResourceConsumption(ResourceScope.Machine)]
        internal StreamReader2(String path, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize, bool checkHost)
        {
            ...
            var fileStreamConstructors = (typeof(FileStream)).GetConstructors(BindingFlags.NonPublic | BindingFlags.Instance);
            Stream stream = fileStreamConstructors[0].Invoke(new object[] { path, FileMode.Open, FileAccess.Read, FileShare.Read, DefaultFileStreamBufferSize, FileOptions.SequentialScan, Path.GetFileName(path), false, false, checkHost }) as FileStream;
        ...
        {
            // Dispose of our resources if this StreamReader2 is closable.
        ...
        // DiscardBufferedData tells StreamReader2 to throw away its internal
        // buffer contents.  This is useful if the user needs to seek on the
        // underlying stream to a known location then wants the StreamReader2
        // to start reading from this new point.  This method should be called
        // very sparingly, if ever, since it can lead to very poor performance.
        // However, it may be the only way of handling some scenarios where 
        // users need to re-read the contents of a StreamReader2 a second time.
        ...
        public override int Read([In, Out] char[] buffer, int index, int count)
        {
            ...
            while (count > 0)
            {
                ...
                if (!readToUserBuffer)
                {
                    Array.Copy(charBuffer, charPos * 2, buffer, (index + charsRead) * 2, n * 2);
                    charPos += n;
                }
                charsRead += n;
                count -= n;
        ...
        {
            Contract.Assert(byteLen >= n, "CompressBuffer was called with a number of bytes greater than the current buffer length.  Are two threads using this StreamReader2 at the same time?");
        ...
        private bool IsPreamble()
        {
            ...
            Contract.Assert(bytePos <= _preamble.Length, "_compressPreamble was called with the current bytePos greater than the preamble buffer length.  Are two threads using this StreamReader2 at the same time?");
            ...
            Contract.Assert(bytePos <= _preamble.Length, "possible bug in _compressPreamble.  Are two threads using this StreamReader2 at the same time?");
        ...
        internal virtual int ReadBuffer()
        {
            ...
            do
            {
                ...
                {
                    Contract.Assert(bytePos == 0, "bytePos can be non zero only when we are trying to _checkPreamble.  Are two threads using this StreamReader2 at the same time?");
        ...
        // user's buffer, bypassing StreamReader2's own buffer.
                    ...
                    if (len == 0)
                    {
                        ...
                        if (byteLen > 0)
                        {
                            if (readToUserBuffer)
                            {
                                ...
                                charLen = 0;  // StreamReader2's buffer is empty.
                            }
                            else
                            {
                                charsRead = decoder.GetChars(byteBuffer, 0, byteLen, charBuffer, charsRead);
                                charLen += charsRead;  // Number of chars in StreamReader2's buffer.
                            }
                        }
        ...
        public override Task<String> ReadLineAsync()
        {
            ...
            if (this.GetType() != typeof(StreamReader2))
            ...
            while (count > 0)
            {
                ...
                if (n == 0)
                {
                    ...
                    do
                    {
                        ...
                        if (CheckPreamble_Prop)
                        {
                            Contract.Assert(BytePos_Prop <= Preamble_Prop.Length, "possible bug in _compressPreamble.  Are two threads using this StreamReader2 at the same time?");
                            ...
                            if (len == 0)
                            {
                                ...
                                if (ByteLen_Prop > 0)
                                {
                                    if (readToUserBuffer)
                                    {
                                        ...
                                        CharLen_Prop = 0;  // StreamReader2's buffer is empty.
                                    }
                                    else
                                    {
                                        n = Decoder_Prop.GetChars(tmpByteBuffer, 0, ByteLen_Prop, CharBuffer_Prop, 0);
                                        CharLen_Prop += n;  // Number of chars in StreamReader2's buffer.
                                    }
                                }
                        ...
                        {
                            Contract.Assert(BytePos_Prop == 0, "_bytePos can be non zero only when we are trying to _checkPreamble.  Are two threads using this StreamReader2 at the same time?");
        ...
        private class NullStreamReader : StreamReader2
        {
